...
async function startServer() {
  ...
  app.post("/api/schools", async (req, res) => {
    try {
      ...
      await pool.query(
        "INSERT INTO history (school_id, action, description, action_date) VALUES ($1, $2, $3, $4)",
        [
          newSchool.id,
          "System",
  ...
  app.put("/api/schools/:id", async (req, res) => {
    try {
      ...
      if (updateResult.rowCount === 0) return res.status(404).json({ error: "School not found" });

      const updated = updateResult.rows[0];

      await pool.query(
        "INSERT INTO history (school_id, action, description, action_date) VALUES ($1, $2, $3, $4)",
        [req.params.id, "System", `School renamed to ${name}`, new Date().toISOString()]
      );
  ...
  app.delete("/api/schools/:id", async (req, res) => {
    try {
      ...
      if (deleteResult.rowCount === 0) return res.status(404).json({ error: "School not found" });
  ...
  app.post("/api/tasks", async (req, res) => {
    try {
      ...
      if (!school_id || !type || !due_date) return res.status(400).json({ error: "Missing required fields" });

      const insertResult = await pool.query(
        "INSERT INTO tasks (school_id, type, due_date, status) VALUES ($1, $2, $3, $4) RETURNING *",
        [school_id, type, due_date, "Pending"]
      );
  ...
  app.put("/api/tasks/:id/status", async (req, res) => {
    try {
      ...
      if (updateResult.rowCount === 0) return res.status(404).json({ error: "Task not found" });
  ...
  app.put("/api/tasks/:id", async (req, res) => {
    try {
      ...
      if (!type || !due_date) return res.status(400).json({ error: "Missing required fields" });

      const updateResult = await pool.query(
        "UPDATE tasks SET type = $1, due_date = $2 WHERE id = $3 RETURNING *",
        [type, due_date, req.params.id]
      );
      if (updateResult.rowCount === 0) return res.status(404).json({ error: "Task not found" });
      res.json(updateResult.rows[0]);
    } catch (err: any) {
  ...
  app.delete("/api/tasks/:id", async (req, res) => {
    try {
      ...
      if (deleteResult.rowCount === 0) return res.status(404).json({ error: "Task not found" });
      res.json({ success: true });
    } catch (err: any) {
      res.status(500).json({ error: err.message });
    }
  ...
  app.get("/api/history/:schoolId", async (req, res) => {
    try {
      const result = await pool.query(
        "SELECT * FROM history WHERE school_id = $1 ORDER BY action_date DESC, id DESC",
        [req.params.schoolId]
      );
      res.json(result.rows);
    } catch (err: any) {
  ...
  app.post("/api/history", async (req, res) => {
    try {
      ...
      if (!school_id || !action || !action_date) return res.status(400).json({ error: "Missing required fields" });

      const insertResult = await pool.query(
        "INSERT INTO history (school_id, action, description, action_date) VALUES ($1, $2, $3, $4) RETURNING *",
        [school_id, action, description, action_date]
      );
      res.status(201).json(insertResult.rows[0]);
    } catch (err: any) {
      res.status(500).json({ error: err.message });
